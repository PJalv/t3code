...
import { enableLocalAgentNotifications } from "../agent-awareness/localNotifications";
...
function ConfiguredSettingsRouteScreen() {
  ...
  // Local (no T3 Connect account) notifications: configured with a device-local
  // identity and delivered by the local watcher instead of the hosted relay.
  const [localNotificationsEnabled, setLocalNotificationsEnabled] = useState(false);
  const [localOngoingEnabled, setLocalOngoingEnabled] = useState(false);
  ...
  // The native handler is re-applied on every signed-out launch, so the switches
  // must read the persisted local choice (or an already-granted permission)
  // instead of defaulting to off after a restart.
  useEffect(() => {
    if (isSignedIn) {
    ...
    void (async () => {
      try {
        ...
        setLocalOngoingEnabled(preferences.localNotificationsOngoing === true);
        if (preferences.localNotificationsEnabled === true) {
          setLocalNotificationsEnabled(true);
          return;
        }
        const permission = await Notifications.getPermissionsAsync();
        setLocalNotificationsEnabled(permission.granted);
      } catch {
        // Leave the switch off if preferences cannot be read.
      }
    })();
  }, [isSignedIn]);
      ...
      if (enabled) {
        if (!isSignedIn) {
          void (async () => {
            ...
            if (permission._tag === "Failure") {
              reportAtomCommandResult(permission, { label: "local notification permission" });
              return;
            }
            if (permission.value.type !== "granted") {
              Alert.alert("Notifications disabled", "Notifications were not enabled.");
            ...
            setNotificationStatus("enabled");
            setLocalNotificationsEnabled(true);
            await enableLocalAgentNotifications(false);
            Alert.alert(
              "Notifications enabled",
              "Local agent notifications are enabled for this device.",
            );
          })();
          return;
        }
    ...
    (enabled: boolean) => {
      ...
      if (!isSignedIn) {
        void (async () => {
          ...
          if (permission._tag === "Failure") {
            reportAtomCommandResult(permission, { label: "local activity permission" });
            return;
          }
          if (permission.value.type !== "granted") {
            Alert.alert(
          ...
          setLiveActivityStatus("enabled");
          setLocalNotificationsEnabled(true);
          setLocalOngoingEnabled(true);
          await enableLocalAgentNotifications(true);
          Alert.alert(
            "Ongoing activity enabled",
            "Local ongoing activity updates are enabled for this device.",
          );
        })();
        return;
      }
        ...
        <SettingsSection title="Configuration">
          ...
          <SettingsSwitchRow
            ...
            value={
              ...
              (deviceRegistered || localNotificationsEnabled)
            }
